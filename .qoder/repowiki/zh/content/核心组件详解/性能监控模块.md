...
`run_throughput_benchmark()` 方法用于测试系统在高负载下的吞吐能力。这些基准测试功能可用于版本对比、性能回归检测和硬件适配评估。基准测试在隔离环境中运行，避免影响生产监控数据。
...
通过分析百分位值、方差和趋势变化，系统能够识别间歇性延迟尖峰、内存泄漏倾向和资源竞争等问题，为深度性能调优提供指导。报告生成过程在后台线程中执行，避免阻塞主监控路径。
